...
function App() {
  ...
  const saveEdit = (id, editName, editDeadline, editCategory, editPriority) => {
    const newUpdateGoal = goals.map((goal) =>
      goal.id === id
        ? {
            ...goal,
            name: editName,
  ...
  const filterGoal = goals.filter((goal) => {
    const StatusMatch =
      filterStatus === "all" ||
      (filterStatus === "complete" && goal.iscomplete) ||
      (filterStatus === "pending" && !goal.iscomplete);

    const priorityMatch =
      filterPriority === "all" ||
      goal.priority.toLowerCase() === filterPriority.toLowerCase();
  ...
  return (
    <div className="goal-container">
      ...
      <div className="goal-list">
        <GoalList
          goals={sortGoals(filterGoal, sortMethod)}
